...
- `pnpm desktop` — Electron shell; dev loop (`apps/desktop/scripts/dev.mjs`: never starts or owns the SPA and never waits for it — resolves `HOARDODILE_WEB_URL` or the default port and launches regardless; when `pnpm dev`'s Vite is absent the shell window shows its own Retry page (start `pnpm dev`, press Retry); starts the wizard on a free port; tree-kills the sidecar on exit), production windows load the sidecar-served SPA. `desktop:package` = Windows x64 NSIS installer + portable zip (does not publish); `desktop:release` = local publish path requiring `GH_TOKEN` (`latest.yml` + blockmap onto the GitHub Release) — a CI workflow for it is not in place yet.
