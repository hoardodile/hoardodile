...
- `pnpm desktop` — Electron shell; one-command dev loop (`apps/desktop/scripts/dev.mjs`: starts or reuses the SPA Vite and the wizard on free ports, default ports from `scripts/lib/dev-ports.json`, tree-kills the sidecar on exit), production windows load the sidecar-served SPA. `desktop:package` = Windows x64 NSIS installer + portable zip (does not publish); `desktop:release` = local publish path requiring `GH_TOKEN` (`latest.yml` + blockmap onto the GitHub Release) — a CI workflow for it is not in place yet.
...
- One app version owned by root `package.json`, synced by `pnpm release`; publish via `pnpm -r publish` right after; **never hand-edit a `version` field**.
