...
- `pnpm desktop` — Electron shell; dev loop (`apps/desktop/scripts/dev.mjs`: reuses an externally started SPA Vite from `pnpm dev` (or `HOARDODILE_WEB_URL`) — never starts/owns it, fails fast if missing; starts the wizard on a free port; tree-kills the sidecar on exit), production windows load the sidecar-served SPA. `desktop:package` = Windows x64 NSIS installer + portable zip (does not publish); `desktop:release` = local publish path requiring `GH_TOKEN` (`latest.yml` + blockmap onto the GitHub Release) — a CI workflow for it is not in place yet.
