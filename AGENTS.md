...
- `pnpm desktop` — Electron shell; dev mode loads the web Vite URL (default 5173, so `pnpm dev` must be running for SPA HMR), production windows load the sidecar-served SPA. `desktop:package` = Windows x64 NSIS installer + portable zip (does not publish); `desktop:release` = local publish path requiring `GH_TOKEN` (`latest.yml` + blockmap onto the GitHub Release) — a CI workflow for it is not in place yet.
