...
- No telemetry or external calls — the only authorized network requests are the user-triggered update check (desktop `autoUpdate` may fetch GitHub Release artifacts while the tray is alive), user-consented plugin downloads (the plugin asset API: every download requires the shared consent dialog in the web UI, and the file lands only inside the plugin's own `vault/` folder), and the plugin marketplace's GitHub fetches (built-in default registry `hoardodile/marketplace`; a custom registry is opt-in): catalog refresh (registry + per-plugin manifests and repo READMEs via raw URLs + `releases/latest` via the API + per-release `intro.<locale>.md` assets via release download URLs, server-side, cached 10 min, on page open or explicit refresh) and user-confirmed install/update downloads of release assets (same hardened HTTP client; GitHub release hosts only).
