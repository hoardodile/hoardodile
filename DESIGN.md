...
- **Dialog anatomy.** The card floats at `bg-card` + hairline + `rounded-2xl` + `shadow-dialog`. The body is the only scrolling region (`flex-1 overflow-y-auto`), header carries `p-5` with no bottom padding, the footer parts from the body with an inset hairline (`mx-5`, never edge to edge), and `gap-4` holds the three parts apart. **Three-button footers** split the bar: the secondary function key sits at the left edge, cancel and the primary action stay right-aligned. **Two-button footers** never split: cancel leads and the secondary function key holds the right edge — the function key is always at an outer edge (left with three actions, right with two). `flush` drops body padding for two-pane editors.
