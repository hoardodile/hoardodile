 ...
 * the two newest snapshots per device are kept — the previous one exists
 * purely so the UI can show what changed since the last record. All
 * values are captured server-side at record time, never user-entered.
 */
