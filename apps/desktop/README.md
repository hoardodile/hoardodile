...
**Desktop dev is one command.** `pnpm desktop` (`apps/desktop/scripts/dev.mjs`) starts the wizard on the first free port at or above the wizard default and the SPA on the first free port at or above the SPA default (bound to `127.0.0.1` — Vite's default `localhost` binding may resolve to `::1` only). If a hoardodile Vite is already served on the SPA port (e.g. `pnpm dev` for a browser tab), it is reused instead of starting a second one. Default ports live in `scripts/lib/dev-ports.json` (SPA / wizard / API relay; the API mirror tracks the server default) — change them there, never in consumers. Set `HOARDODILE_WEB_URL` to reuse a SPA on a non-standard port; the script waits for it and fails fast if it is unreachable. The sidecar is spawned by the shell itself (vite-node on `apps/server` source), so no backend or plugin watchers from `pnpm dev` are needed. When the script owns the SPA it points the SPA's own proxy target at a live 503 responder (`VITE_SERVER_URL`); API calls only work inside the desktop window (dest proxy) — a browser tab shows the UI without data. Ctrl+C closes the wizard + SPA servers and tree-kills Electron so the sidecar does not linger.
