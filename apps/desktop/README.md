...
**Desktop dev is two terminals, one process each.** `pnpm dev` in one terminal gives the SPA (Vite on the SPA default port) plus the usual web/backend loop; `pnpm desktop` in the other (`apps/desktop/scripts/dev.mjs`) never starts or owns the SPA — it probes the SPA default for an already-served hoardodile Vite, waits (≈20s), and fails fast with a fix-the-command message otherwise. Set `HOARDODILE_WEB_URL` to reuse a SPA on a non-standard port; the script waits for reachability and fails fast if it is unreachable. The script starts the wizard on the first free port at or above the wizard default (bound to `127.0.0.1` — Vite's default `localhost` binding may resolve to `::1` only), one-shot builds main + preload, then spawns Electron. Default ports live in `scripts/lib/dev-ports.json` (SPA / wizard; the API mirror tracks the server default) — change them there, never in consumers. The sidecar is spawned by the shell itself (vite-node on `apps/server` source), so no backend or plugin watchers from `pnpm dev` are needed. `VITE_SERVER_URL` and the SPA's proxy target are never touched by the script; a browser tab keeps its normal `pnpm dev` routing, and API calls inside the desktop window reach the sidecar through main's dest proxy. Ctrl+C closes the wizard server and tree-kills Electron so the sidecar does not linger.
