 ...
 *  1. Copy the sandbox worker entry into dist/chunks. The sandbox module
 *     lives in exactly one shared chunk at `dist/chunks/host.js` (fixed
 *     chunk names) and spawns the worker by relative URL, so the entry
 *     must sit next to it — worker threads get no transforms, it ships as
 *     plain ESM exactly as authored.
 *  2. Dist-level smoke: spawn a REAL worker from dist and run `detect`
 *     against a fixture plugin. This is the cheapest guard against the
 *     sandbox silently dying in published artifacts (missing worker entry,
 *     broken protocol import, bundled-away node builtins).
 *
...
try {
	...
	console.log("[host:postbuild] dist smoke passed (real worker, detect ok)")
} finally {
