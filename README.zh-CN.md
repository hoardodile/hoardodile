...
- 自定义插件：将你的插件目录挂载到 `/app/plugins/<slug>`（不要覆盖内置种子插件），然后在界面中安装。若要分发自己的插件，用 GitHub Release 发布并把“设置 → 插件市场”指向你的注册单仓库（内置默认为 `hoardodile/marketplace`）即可——见 `packages/cli/README.md`。
